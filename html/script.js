window.addEventListener("message", function (event) {
    ...
    if (data.action === "VehicleList") {
        ...
        displayUI();
    } else if (data.action === "VehicleListJobs") {
        const garageLabel = data.garageLabel;
        const vehicles = data.vehicles;
        populateVehicleListJobList(garageLabel, vehicles);
        displayUI();
    }
});
...
function populateVehicleListJobList(garageLabel, vehicles) {
    ...
    vehicles.forEach((v) => {
        ...
        if (v.balance && v.balance > 0) {
            financeInfo.textContent = v.playerjobadndgrade;
        } else {
            financeInfo.textContent = v.playerjobadndgrade;
        }
        ...
        driveButton.onclick = function () {
            ...
            if (status === "Drive") {
                fetch("https://qb-garages/takeOutVehicleJobs", {
                    method: "POST",
                    headers: {
                        "Content-Type": "application/json; charset=UTF-8",
                    },
